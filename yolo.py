...
def detect_objects(image):
    ...
    return results[0].plot(), counts  # Return image with bounding boxes + detection summary
        ...
        with col2:
            ...
            if total_count > 0:
                ...
                def autopct_jitter(pct):
                    if pct > 2:
                        jitter_x = np.random.uniform(-0.1, 0.1)  # Small x offset
                        jitter_y = np.random.uniform(-0.1, 0.1)  # Small y offset
                        return f"{pct:.1f}%"
                    return ""
                ...
                # Apply jitter effect manually
                for autotext in autotexts:
                    autotext.set_position(
                        (autotext.get_position()[0] + np.random.uniform(-0.1, 0.1),
                         autotext.get_position()[1] + np.random.uniform(-0.1, 0.1))
                    )
                    autotext.set_fontsize(10)  # Ensure readable size
